...
Reynolds-averaged Navier--Stokes numerical model. Note that studies in the
literature generally focus on predicting the turbine loading and the local blade
flow rather than the turbine wake. In contrast, we seek to model the near-wake
flow.

The wake of this particular turbine is of interest since it has been shown that
the neark-wake's momentum and energy transport processes are dominated by
vertical advection \cite{Bachant2015-JoT}. It logically follows that a 2-D
simulation would not correctly predict wake recovery, but is of interest to what
extent this is true, since the computational feasibility of 2-D simulations is
attractive.
...
% In this section, show mean velocity contours/quivers
...
% Compare contours of TKE for 2 simulation cases and experiments
\begin{figure}
    \centering
    \begin{subfigure}[b]{\textwidth}
        \centering 
