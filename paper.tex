...
Computational fluid dynamics (CFD) presents an attractive method for simulating
cross-flow turbines (CFTs) since it only requires turbulence modeling, i.e., the
machine is described from first principles. However, CFD can be computationally
...
with widely varying results. Balduzzi et al. provides a summary of recent
computations and an attempt to standardize a methodology for using
Reynolds-averaged Navier--Stokes (RANS) to correctly simulate performance of a
2-D CFT \cite{Balduzzi2016}.
...
In this study we seek to predict the near-wake of the UNH-RVAT turbine,
experimental performance curves for which are shown in
Figure~\ref{fig:exp_perf}. We modeled the turbine at a tip speed ratio
...
\todo[inline]{Find some examples in the literature where time steps are much
    larger.}
...
3-D Spalart--Allmaras case in Figure~\ref{fig:vorticity-3D}. It can be seen how
the upstream blade is shedding a large amount of spanwise vorticity due to the
separated flow. Strong tip vortices are also present, which trace the
``contracting'' wake flow on the $-y$ side of the turbine associated with the
induced vertical velocity field.
