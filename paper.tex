...
Turbulence kinetic energy profiles are shown in Figure~\ref{fig:k-profiles}.
Note that there are two profiles for each---one computed from the components
from the Reynolds stress tensor $R$:

\begin{equation}
k = \frac{1}{2} \left( R_{xx} + R_{yy} + R_{zz} \right);
\label{eq:k}
\end{equation}
and one that includes unsteadiness in the Reynolds-averaged velocity
\begin{equation}
k_{\mathrm{tot}} = k + \frac{1}{2} \left( 
\overline{U^\prime}^2 + 
\overline{V^\prime}^2 + 
\overline{W^\prime}^2 \right),
\label{eq:ktot}
\end{equation}
where $U^\prime = U - \overline{U}$.
